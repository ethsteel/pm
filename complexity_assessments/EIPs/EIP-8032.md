...
|---|---:|---|
| **EVM Gas rule changes** | 3 | Increases complexity of SSTORE gas cost which poses a challenge for our existing tests (especially static tests that have not been ported yet).  |
| **Blob gas accounting changes** | 0 |   |
| **New EVM gas refund** | 0  |  |
| **Patterns affecting pre-existing tests** | 3 | Any test that uses SSTORE is potentially affected. |
| **Transition-tool interface changes** | 0  |   |
| **Cryptography-related testing** | 0 |   |
| **Edge/boundary conditions** | 3 | Due to varying database implementations and sync modes, the proposed transition process is risky due to edge and boundary conditions. |
...
| **Added opcodes** | 0  |   |
| **Modified opcodes** |  1 |  The cost of SSTORE can be hard to predict with this EIP. |
| **Added precompiles** | 0  |   |
| **Modified precompiles** | 0  |   |
| **Encoding changes (RLP/SSZ)** |  3  | Adds new field for account RLP in state to keep track of amount of storage slots a contract has.   |
| **New transaction types** |  0  |   |
| **New or modified transaction validity mechanisms** |  |  |
| **New block / header fields** | 0 | New field for account RLP in state though. |
| **New fork activation mechanism** | 2  | Introduces a transition process that should be pre-run by clients to prepare for new fork activation.  |
| **Performance risks** |  3  | The proposed transition process is computationally expensive (IO heavy).   |
| **Security risks** | 3  | Due to varying database implementations and sync modes, the proposed transition process is potentially error-prone.  |
...
Since this EIP bundles many EVM gas cost changes into one, they have to be factored multiple times in the same row.
See each row for details.
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`21`** |
