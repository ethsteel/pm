...
Link: https://eips.ethereum.org/EIPS/eip-8032
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | New dynamic gas formula for SSTORE based on `storage_count`. Affects existing SSTORE gas accounting and existing gas tests. |
| **Blob gas accounting changes** | 0 | None  |
| **New EVM gas refund** | 0 | None  |
| **Patterns affecting pre-existing tests** | 2 | All SSTORE gas tests, storage-heavy contract tests, and gas calculation tests require significant rework. |
| **Transition-tool interface changes** | 2 | Introduces a new migration mechanism with multiple persistent cursors that must be tracked by the state transition tool. |
| **Cryptography-related testing** | 0 | None  |
| **Edge/boundary conditions** | 3 | Introduces multiple boundary cases around snapshot timing, partial transitions, and large storage accounts, though none require combinatorial explosion.|
| **Block syncing changes** | 0 | None  |
| **Engine API changes** | 0 | None  |
| **Engine API encoding changes** | 0 | None  |
| **Added system contracts** | 2  | Introduces a new stateful system contract used to persist migration progress.|
...
| **New fork activation mechanism** | 3 | Introduces a fork-activated, block-bounded state transition that modifies account state post-fork.|
| **Performance risks** | 2 | Migration cannot be fully benchmarked in isolation and may affect state access patterns, but impact is bounded per block. |
| **Security risks** | 2 | Introduces new stateful accounting that interacts with gas pricing and storage semantics, requiring targeted review and fuzzing. |
| **Cross-EIP interactions** | 2 | Interacts with existing storage-related EIPs (e.g. gas refund) |
...
The block-bounded account transition introduces a long-lived migration process whose correctness depends on cursor persistence, deterministic trie iteration order, and fork-boundary handling, all of which require dedicated transition and sync tests.
...
- Testing Infrastructure: This eip requires test infrastructure that can simulate multi-block transitions and verify intermediate states. The execution-specs testing framework (`BlockchainTestFiller`) supports multi-block tests, but transition-specific assertions may need development.
- The separation between migration logic and runtime gas semantics simplifies reasoning but requires explicit test coverage to ensure snapshot isolation across block boundaries.
