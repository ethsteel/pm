...
Link: https://github.com/ethereum/EIPs/blob/73e29446093f77c56e98e355a610ed3e32112657/EIPS/eip-8037.md
...
|---|---:|---|
| **EVM Gas rule changes** | 2 + 2 + 3 + 1 | GAS_CREATE + GAS_CODE_DEPOSIT can be considered the same category of change; CALL by itself requires testing must be separated; SSTORE requires its own testing plus the new multidimensional metering logic; EOA changes are simple and reduced in scope. |
| **Blob gas accounting changes** |   |   |
| **New EVM gas refund** | 2 | Refund tests are indirectly affected by the SSTORE change. |
| **Patterns affecting pre-existing tests** | 3 + 3 + 3 | CREATE, CALL, and SSTORE are values that have not been updated since Frontier. This change will break an excessive amount of static tests (https://github.com/ethereum/execution-specs/tree/forks/osaka/tests/static) and will require some rework in existing python tests. Counting each of these as a multiplier for this row. |
| **Transition-tool interface changes** |   |   |
| **Cryptography-related testing** |   |   |
| **Edge/boundary conditions** | 3 | Bundling all changes into this single row since they are similar. |
| **Block syncing changes** |   |   |
| **Engine API changes** |   |   |
...
| **Encoding changes (RLP/SSZ)** |   |   |
| **New transaction types** |   |   |
| **New or modified transaction validity mechanisms** | 3 + 1 | Modifies the contract creation from a contract-creating transaction, and the EOA delegation cost. |
| **New block / header fields** |   |   |
| **New fork activation mechanism** |   |   |
| **Performance risks** |   |   |
| **Security risks** |   |   |
| **Cross-EIP interactions** | 2 | Considering contract creation and account calling are so fundamental, this will require oversight on other EIPs that are CFI'd. |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`28`** |
