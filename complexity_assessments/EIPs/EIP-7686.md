...
Link: https://github.com/ethereum/EIPs/blob/1d2e1c18f7cdecc8fa35a74a0040c61ed326a9c1/EIPS/eip-7686.md

## Execution Specs

### Specs

[Finalized EELS documentation/reference prototype](https://github.com/spencer-tb/execution-specs/pull/59), based on `forks/amsterdam` and activated through the testing-side Bogota pseudo-fork.
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | Introduces a hard per-frame memory cap and a memory-aware child-gas rule while replacing quadratic memory pricing. The prototype initially flipped 685 executions across 26 existing test functions. |
| **State-access ordering within opcode execution** | 0 | No state access or gas-charge site moves relative to a state access; the new memory check remains in the existing pre-access memory-pricing phase. |
| **Blob gas accounting changes** | 0 | No blob gas accounting changes. |
| **State gas accounting changes** | 0 | State-gas costs, charging sites, reservoirs, and spill rules are unchanged; only the composition of an execution-gas memory cap with EIP-8037 is tested. |
| **New EVM gas refund** | 0 | No refund mechanism is added or changed. |
| **Patterns affecting pre-existing tests** | 2 | Existing exact-gas, quadratic-memory, EIP-150 forwarding, EIP-7928 BAL, EIP-8037, and ported static scenarios require fixes or re-derivation across multiple suites, but they are all gas- or memory-tuned test categories. |
| **New invariant on pre-existing tests** | 0 | Existing tests do not gain a mechanically additional assertion; affected tests instead need their gas and success boundaries reworked. |
| **Transition-tool interface changes** | 0 | No transition-tool input or result field changes. |
| **New test-framework primitives** | 3 | Adds a permanent fork-level `memory_grant_floor` API used by the generic gas-test generator, EIP-2780 tests, and ported gas measurement tests. |
| **Cryptography-related testing** | 0 | No cryptography changes. |
| **Edge/boundary conditions** | 3 | Linear word rounding, exact/one-past frame limits, post-charge call grants, zero clamping, CALL/CREATE contexts, stipend grants, and state-gas composition create several elevated boundary matrices. |
| **Block syncing changes** | 0 | No block RLP or syncing changes. |
| **Engine API changes** | 0 | No Engine API changes. |
| **Added system contracts** | 0 | No system contracts are added. |
| **Modified system contracts** | 0 | No system contract code, state, or demonstrated system-contract behavior is modified. |
| **Added opcodes** | 0 | No opcodes are added. |
| **Modified opcodes** | 3 | Every memory-expanding opcode can now exceptionally halt at a frame-grant boundary, and CALL/CREATE-family child grants change beyond gas-price changes alone. |
| **Added precompiles** | 0 | No precompiles are added. |
...
| **New or modified transaction validity mechanisms** | 0 | Transaction validity and intrinsic gas are unchanged; a top-frame cap failure is an EVM exceptional halt. |
| **New block / header fields** | 0 | No block or header fields are added. |
| **New fork activation mechanism** | 0 | Activation does not modify state or an existing internal variable at the fork block. |
| **Performance risks** | 2 | The EIP changes maximum allocatable memory and the nested-call memory invariant, so worst-case allocation and recursion are not fully isolated by one gas microbenchmark; no broad existing benchmark rewrite was demonstrated. |
| **Security risks** | 2 | Incorrect cap or child-grant accounting can break the intended resource bound or consensus, requiring targeted fuzzing across memory expansion and nested calls. |
| **Unspecified behavior requiring cross-client consensus** | 2 | The EIP needs localized agreement on reachable negative `max_call_gas`, stipend accounting, and what “initial gas limit” means when composed with EIP-8037; there is no client/devnet baseline. |
| **Cross-EIP interactions** (uncapped) | 3 | Coordinated cases are required for EIP-150 forwarding, EIP-2780 exact-gas creation, EIP-7928 BAL boundaries, and EIP-8037 execution/state gas composition. Four interacting EIPs do not trigger the uncapped increment. |

**Total: 23**

#### Special Considerations

The matrix grows multiplicatively across 18 memory-expansion sites, four CALL-family and two CREATE-family contexts, word-rounding and exact-gas boundaries, value/stipend behavior, 63/64-versus-memory-dominated forwarding, cold/warm and delegated/direct access, and success/revert/exception outcomes. This is not added again to the score. The measured pre-mitigation blast radius was 685 executions across 26 distinct existing test functions; 303 executions came from a generic constant-gas opcode harness rather than EIP-specific tests.

#### Notes

- Final validation at Bogota produced 64,853 passing and 294 skipped fixtures; the EIP-focused suite produced 111 passing fixtures.
- Memory-tuned quadratic-cost and EIP-150 boundary cases are parked with searchable `TODO(EIP-7686)` markers until their post-EIP expectations are re-derived. Complete behavior-changing transition bodies are also skipped until a real post-Amsterdam spec fork exists.
- The prototype clamps a negative child grant to zero, counts the 2,300-gas value stipend in the child's cap, and binds the cap to EIP-8037 execution gas rather than its state-gas reservoir. These are explicit evidence-backed assumptions, not accepted EIP amendments.
- EIP-7923 is a competing page-based memory proposal. It is not added to the cross-EIP score because these alternatives would require a fork-choice decision rather than coordinated activation testing.
...
| **Complexity Tier** | Computed from total score | 🔴 **High Complexity** |
...
Not applicable; EIP-7686 is an execution-layer EVM change.
...
No consensus-layer testing impact was identified.
