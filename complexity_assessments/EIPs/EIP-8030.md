...
Link: https://eips.ethereum.org/EIPS/eip-8030
...
|---|---:|---|
| **EVM Gas rule changes** | 1 | Introduces penalty of 500 gas when this signature algorithm is used. |
| **Blob gas accounting changes** | 0  |   |
| **New EVM gas refund** | 0  |   |
| **Patterns affecting pre-existing tests** | 3  | Previously written tests assume that secp256k1, not secp256r1 is used for signatures. Retrospectively adding a new case that needs to be checked for throughout the framework, which even affects the gas cost, is tremendous testing overhead.  |
| **Transition-tool interface changes** | 0  |   |
| **Cryptography-related testing** | 1  | secp256r1 exists in clients already and is a well known algorithm.  |
...
| **New or modified transaction validity mechanisms** | 3  |  Having to add another check throughout the framework that determines the signature algorithm used which may or may not affect gas cost requires a larger review of the existing testing framework.  |
| **New block / header fields** | 0  |   |
| **New fork activation mechanism** | 0  |   |
| **Performance risks** |  1 |  The EIP author notes that some performed tests show that verifying secp256r1 signatures can be slower than verifying secp256k1 signatures. It is unclear whether 500 gas is a reasonable cost increases and verifying this chosen value will add even more overhead. |
| **Security risks** | 1  | The EIP increases the overall complexity of verifying signatures which in itself brings non-zero security risk.  |
| **Cross-EIP interactions** | 1  |  Introduces new gas-related edge cases (e.g. 500 added gas might be the difference between tx gas limit being overshot or not). |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`11`** |
