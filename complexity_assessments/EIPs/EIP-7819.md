...
Link: https://github.com/ethereum/EIPs/blob/d420fc4b289e298682006b2ea09355065cf50f99/EIPS/eip-7819.md

## Execution Specs

### Specs

The finalized [EELS reference prototype](https://github.com/spencer-tb/execution-specs/pull/57) implements the two-input EIP revision pinned above in Amsterdam and activates it through the testing-side Bogota pseudo-fork. The EIP's legacy EIP-7702 gas constants do not exist on that base, so the prototype translates them into EIP-8037/8038 access, write, account-creation, designation-state, and state-gas-refill components. That translation and the other consensus-visible assumptions are documented as required EIP follow-up in the implementation PR.
...
|---|---:|---|
| **EVM Gas rule changes** | 2 | `SETDELEGATE` introduces a dynamic schedule composed of cold/warm access and an account-write charge. It does not change another opcode's schedule, but exact and repeat-write cases need dedicated accounting tests. |
| **State-access ordering within opcode execution** | 2 | A new state-accessing opcode must check state-independent access gas before reading the location, then calculate and charge state-dependent costs. BAL output changes at those boundaries. |
| **Blob gas accounting changes** | 0 | No blob gas behavior changes. |
| **State gas accounting changes** | 2 | The prototype adds a new `NEW_ACCOUNT`/`AUTH_BASE` state-gas charging site and a journaled designation refill on clear or frame rollback. It reuses the reservoir and spill mechanism rather than changing it. |
| **New EVM gas refund** | 1 | The current EIP text specifies a simple existence-based global refund. The Amsterdam prototype replaces it with EIP-8037 state-gas accounting, so the normative disposition of that refund still needs agreement. |
| **Patterns affecting pre-existing tests** | 1 | Three fixture executions of one existing constant-gas opcode-sweep function flipped because `SETDELEGATE` is dynamic; the sweep now excludes it. No other existing test was reworked or parked. |
| **New invariant on pre-existing tests** | 0 | Tests unrelated to EIP-7819 gain no new assertion. |
| **Transition-tool interface changes** | 0 | The transition-tool request and result interfaces are unchanged. |
| **New test-framework primitives** | 1 | The framework gains one opcode definition with dynamic metadata and a reusable address-derivation helper; existing fillers and expectation types suffice. |
| **Cryptography-related testing** | 0 | Address derivation reuses existing Keccak-256 and introduces no new cryptographic primitive. |
| **Edge/boundary conditions** | 3 | Testing spans target truncation/padding, salt/address order, zero-target clearing, nonce zero/nonzero, exact and malformed collision code, cold/warm access, fresh/existing leaves and designations, exact/one-short gas, static mode, call contexts, and success/revert interactions. Several axes combine in elevated gas/state matrices. |
| **Block syncing changes** | 0 | Block encoding and RLP validation are unchanged. |
| **Engine API changes** | 0 | No Engine API field or endpoint changes. |
| **Added system contracts** | 0 | No system contract is added. |
| **Modified system contracts** | 0 | No existing system contract is modified directly or indirectly. |
| **Added opcodes** | 2 | One complex opcode is added: it has dynamic access, write, and state-gas costs and mutates another account while returning its derived address. |
| **Modified opcodes** | 0 | No pre-existing opcode's result changes. |
| **Added precompiles** | 0 | No precompile is added. |
| **Modified precompiles** | 0 | No precompile logic or gas schedule changes. |
| **Encoding changes (RLP/SSZ)** | 0 | No transaction, block, or interface encoding changes. |
| **New transaction types** | 0 | No transaction type is introduced. |
| **New or modified transaction validity mechanisms** | 0 | Transaction validity and intrinsic gas rules are unchanged. |
| **New block / header fields** | 0 | No block or header field is added. |
| **New fork activation mechanism** | 0 | Activation only makes the opcode valid; it does not mutate state at the fork block. |
| **Performance risks** | 1 | The opcode adds account reads and code/nonce writes, but it can be benchmarked in isolation and does not alter existing benchmark behavior. |
| **Security risks** | 2 | The opcode creates and upgrades immediately effective EIP-7702 designations and interacts with collision, nonce, delegation-resolution, and `SELFDESTRUCT` invariants. These interactions are limited but warrant targeted review and fuzzing. |
| **Unspecified behavior requiring cross-client consensus** | 2 | The current EIP leaves localized consensus questions around post-EIP-8037 gas, operand order, prefix-only versus exact designation collisions, and the state point used for trie existence. The prototype records assumptions, but clients need agreement before production vectors are baselined. |
| **Cross-EIP interactions** (uncapped) | 3 | Coordinated cases cover EIP-7702 designation semantics, EIP-2929 warming, EIP-7523 nonce persistence, EIP-3541 collision reachability, EIP-6780 `SELFDESTRUCT`, EIP-7928 BAL observability, EIP-8037 state gas, and EIP-8038 access/write pricing. The base interaction level is 2; eight interacting EIPs add 1 for the five beyond the first three. |

**Total: 22**

#### Special Considerations

State-access ordering makes the current matrix multiplicative: cold/warm status, fresh/existing state, designation/no designation, direct/delegated execution context, static/non-static execution, and success/revert outcomes can change different observable boundaries. The prototype selects explicit specimens across these axes rather than multiplying every combination blindly.

#### Notes

- The finalized suite has 21 distinct test functions. Bogota filling selected and passed 104 fixture executions; the complete fork-transition body remains skipped until a real post-Amsterdam spec fork exists.
- Interaction validation also passed 1,119 all-opcode fixture executions and 292 EIP-8037 reservoir fixture executions, and `just static` passed.
- No existing tests are parked. The only pre-existing change is the diagnosed dynamic-opcode exclusion from the constant-gas sweep.
- The implementation is pinned to EIP revision `d420fc4b289e298682006b2ea09355065cf50f99` and explicit Amsterdam base `263606b3cd0a352112697870c2c878e2423546b7`.
- The open trailing-data amendment changes the opcode to four inputs and introduces memory, payload-length, and code-data gas axes. If adopted, the implementation and this score must be redone against that revision.

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`22`** |
| **Complexity Tier** | Computed from total score | 🟡 **Medium Complexity** |
