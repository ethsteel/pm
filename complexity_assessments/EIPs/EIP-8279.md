...
|---|---:|---|
| **EVM Gas rule changes** | 3 | A new floor mechanism folded into `tx.gasUsed = max(execution_gas_used, floor_gas_used)`, extending the floor family that the eip7623/eip7976/eip7981 suites actively test; their boundary cases shift and any gas-exact test heavy in cold accesses can flip. |
...
| **Patterns affecting pre-existing tests** | 2 | The floor rarely binds for typical transactions, but EIP-7623/7981/8131 floor tests need rework and cold-access-heavy gas-exact tests can flip; a considerable but contrived category. |
| **New invariant on pre-existing tests** | 0 |    |
| **Transition-tool interface changes** | 0 |    |
| **New test-framework primitives** | 2 | Floor calculators extend the intrinsic-cost machinery: static seed (incl. the per-authorization BAL term) plus a BAL-byte model for expected `gasUsed`; reusable within the suite. |
...
| **Unspecified behavior requiring cross-client consensus** | 2 | The interaction with EIP-8037's two-dimensional gas is unwritten: the floor is defined over execution gas while state gas spills into it, and `execution_gas_used` under reservoir/spill needs pinning before tests are baselined. |
...
Written against a pre-8037 gas model; the Hegota rebase must define the floor's relationship to state gas and the spill path before implementation.

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`26`** |
