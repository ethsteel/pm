...
Link: https://eips.ethereum.org/EIPS/eip-8279
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | A new floor mechanism folded into `tx.gasUsed = max(execution_gas_used, floor_gas_used)`, extending the floor family that the eip7623/eip7976/eip7981 suites actively test. Measured: an EELS prototype flips 384 fixture executions across the eip7623/7976/2780/8037/7778/7934/7954 gas suites. |
| **State-access ordering within opcode execution** | 3 | `meter_bal_data` MUST run before every BAL insertion across the whole class of state-accessing opcodes, adding a new abort point positioned relative to state accesses; every gas boundary of every affected opcode acquires a new observable. Multiplicative case growth — see Special Considerations. |
| **Blob gas accounting changes** | 0 |    |
| **State gas accounting changes** | 0 |    |
| **New EVM gas refund** | 1 | The −32-byte meter refund for restored slots is deliberately distinct from the EIP-3529 gas refund and never touches the refund counter; a simple mechanism whose complexity is already scored under the gas-rule and ordering rows. |
| **Patterns affecting pre-existing tests** | 2 | The floor rarely binds for typical transactions. Measured: 384 of ~61,800 fixture executions fail, but they collapse to 11 test functions, all in floor/gas-boundary suites — a considerable but contrived category, dominated (336) by the two calldata-floor `test_transaction_validity` functions whose exact-boundary transactions now abort. |
| **New invariant on pre-existing tests** | 0 |    |
| **Transition-tool interface changes** | 0 |    |
| **New test-framework primitives** | 1 | The intrinsic/data-floor calculators already model the EIP-7623 floor family; they gain the per-authorization static seed term and byte-floor constants. Measured: the prototype suite needed only a small test-local scaffold helper beyond those extensions. |
| **Cryptography-related testing** | 0 |    |
| **Edge/boundary conditions** | 3 | Nine runtime trigger rows x cold/warm x revert behavior x slot charge/refund toggling x the exact floor-binding boundary (`tx.gas == floor`) x the 7702 static seed — at least one mechanism with an elevated case count. |
...
| **New or modified transaction validity mechanisms** | 2 | Validation becomes `tx.gas >= max(intrinsic, static_floor)` with the new per-auth BAL term; existing validity tests in the 8131/7981 family need limited updates. |
| **New block / header fields** | 0 |    |
| **New fork activation mechanism** | 0 |    |
| **Performance risks** | 2 | The meter itself is cheap, but the EIP redefines worst-case block composition — the bloatnet/worst-case benchmark suites that target exactly these vectors must be re-derived. |
| **Security risks** | 2 | A DoS-bound mechanism: an implementation error either reopens the 1.5 MB block-size bypass or aborts valid transactions; interacts with core gas metering; targeted review and fuzzing. |
| **Unspecified behavior requiring cross-client consensus** | 2 | Two details need pinning before baselining. The interaction with EIP-8037's two-dimensional gas is unwritten (the prototype's obvious reading — the byte floor extends the calldata floor inside the existing settlement `max` — worked, but clients must agree). And whether an access that aborts on the floor check leaves its bytes counted is unspecified; counting them lets a reverted subcall force `floor > gas_limit` at settlement, so the access must not commit — observable at every floor boundary. |
| **Cross-EIP interactions** (uncapped) | 4 | Requires EIP-8131 (itself chaining 7623/7976/7981); meters EIP-7928's bytes; static seed couples to EIP-7702; triggers couple to EIP-2929 cold tracking; the floor must be reconciled with EIP-8037; and EIP-4758 changes the SELFDESTRUCT-beneficiary trigger. Six interacting EIPs: 3 base + 1 increment. |

**Total: 25**

#### Special Considerations

The State-access ordering anchor's multiplicative warning applies in full: each metering trigger must be exercised against every dimension that changes the answer (cold/warm, revert/success, charge/refund toggling, floor binding or not), so the case count grows multiplicatively rather than additively. The EIP is also the only one of the BAL family whose correctness question is quantitative (worst-case block bytes), so its test plan includes adversarial block-composition benchmarks, not just state tests.

#### Notes

Scores calibrated against a full EELS prototype (byte meter at every recordable-access site plus the settlement fold, on a Bogota development fork); the measurements cited above come from a full-suite fixture fill against it. The EIP text predates EIP-8037, and the Hegota rebase must still write down the floor's relationship to state gas.

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`25`** |
