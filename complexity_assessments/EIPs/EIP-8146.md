...
This analysis covers the execution layer only.
...
Implementation finding (execution-specs#3387): the EIP's "`engine_newPayloadV5` unchanged" holds only if EIP-8146 ships in the same fork as EIP-7732/7928 — shipping after a live Glamsterdam freezes V5's shape and forces a version bump (`engine_newPayloadV6` / `engine_getPayloadV7`), which the engine test dimensions inherit.
