...
Link: https://github.com/ethereum/EIPs/blob/69e53477cb79ececd493856354323e1dc10f4094/EIPS/eip-7609.md
...
|---|---:|---|
| **EVM Gas rule changes** | 2 | New gas accounting mechanism: TSTORE gains an allocation-count term, `SLOPE * len(transient_mapping)` on first write to a key. The rule itself is simple and linear, but clients must now track `len(transient_mapping)`, which nothing prices against today. Whichever scope is settled on will likely contradict what some clients already do — EELS keys transient storage in a single transaction-wide dict — so expect implementation rework, not just a new constant. |
| **State-access ordering within opcode execution** | 0 | Transient storage is not consensus state and is explicitly excluded from the EIP-7928 block-level access list, so there is no ordering to settle. |
| **Blob gas accounting changes** | 0 |  |
| **State gas accounting changes** | 0 | Transient storage is wiped per transaction and never persists, so no state gas applies. |
| **New EVM gas refund** | 0 |  |
| **Patterns affecting pre-existing tests** | 1 | Blast radius measured, not estimated: 36 existing tests fail after a first-pass implementation in EELS. |
| **New invariant on pre-existing tests** | 0 |  |
| **Transition-tool interface changes** | 0 |  |
| **New test-framework primitives** | 0 | Existing bytecode helpers and gas assertions suffice to write N distinct slots and assert cumulative cost. |
| **Cryptography-related testing** | 0 |  |
| **Edge/boundary conditions** | 1 | Multiple boundaries within one pricing mechanism, none needing elevated cases. |
| **Block syncing changes** | 0 |  |
| **Engine API changes** | 0 |  |
| **Added system contracts** | 0 |  |
| **Modified system contracts** | 0 |  |
| **Added opcodes** | 0 |  |
| **Modified opcodes** | 1 | Semantics are untouched, but TSTORE cannot be implemented as a gas-constant swap: pricing needs a per-contract count of live transient slots, which the committed state interface cannot supply. `transient_storage` is a flat `Dict[Tuple[Address, Bytes32], U256]`, so a per-address count means scanning a transaction-wide dict. A reference implementation restructured it to nested per-address dicts and added `get_transient_storage_size`, +43/-6 in `state_tracker.py` alone. Scored 1 as a deliberate departure: this anchor is binary 0/3 and excludes gas changes, so it has no level for an opcode that keeps its semantics but needs new state machinery. The pricing rule itself is scored under EVM Gas rule changes. |
| **Added precompiles** | 0 |  |
...
| **Performance risks** | 1 | Existing `tests/benchmark/stateful/bloatnet/test_transient_storage.py` benchmarking test needs an update to account for the new gas mechanism. |
| **Security risks** | 0 |  |
| **Unspecified behavior requiring cross-client consensus** | 2 | 1. Revert semantics: EIP-1153 rolls transient storage back on revert, but the EIP does not say whether the size used for `expansion_cost` rolls back with it. 2. Scope: whether the mapping is per contract or per transaction is never stated, so it has to be settled by discussion before any vector can be baselined. The downstream implementation cost is scored under EVM Gas rule changes. 3. Zero writes: it is not stated whether setting a slot to zero decrements the mapping size. EELS pops the key today, so as implemented it does. |
| **Cross-EIP interactions** (uncapped) | 0 | None. |
...
- Status is Stagnant (created 2024-02-01). Requires only EIP-1153.
- `SLOPE` is 1 gas per already-allocated slot, so cost per new slot grows without bound within a transaction; there is no cap other than gas exhaustion.
- The EIP cites benchmarking (TLOAD comparable to `MUL`, TSTORE about 1.5x) but gives no data or methodology. Worth reproducing before accepting 5 and 8.
- The mapping **can** shrink: `set_transient_storage` in `src/ethereum/forks/amsterdam/state_tracker.py` pops the key when the value is zero. So `len(transient_mapping)` is directly manipulable by writing zeros, which bounds concurrent size but not total allocation churn.
- The committed baseline keys transient storage as a flat `Dict[Tuple[Address, Bytes32], U256]`, so the only size cheaply available today is transaction-wide. Obtaining the per-contract count the EIP's prose implies requires restructuring the state interface, not just reading it.
