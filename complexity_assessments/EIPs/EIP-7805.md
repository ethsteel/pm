...
Link: https://eips.ethereum.org/EIPS/eip-7805
...
| **Patterns affecting pre-existing tests** | 1 | "After all of the transactions in the payload have been executed, we check whether any transaction from ILs, that is not already present in the payload, could be validly included...". Additionally, there is further validation for each Transaction in the IL  |
| **Transition-tool interface changes** | 2 | new field, new mechanism for tx validation, new error  |
| **Cryptography-related testing** | 0 |   |
| **Edge/boundary conditions** | 2 | Many boundary/edge cases to test, but standard checks |
| **Block syncing changes** | 0 |   |
| **Engine API changes** | 3 | 2 new engine apis included, 1 modified |
...
| **New or modified transaction validity mechanisms** | 2 | Need to validate each transation in the IL, but limited in scope and straightforward checks |
| **New block / header fields** | 0 |   |
| **New fork activation mechanism** | 0 |   |
| **Performance risks** | 2 | Each transaction not in block requires state access, adding latency to block validation time  |
| **Security risks** | 2 | Consensus-critical validations, new economic incentives |
| **Cross-EIP interactions** | 1 | Interacts with AA |

**Total: 15**

#### Special Considerations

This analysis has only been done for execution layer clients. The impact on CL clients will be much greater.
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`15`** |
