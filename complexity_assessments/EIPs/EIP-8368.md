...
| **Performance risks** | 1 | No new mechanism, but the recalibrated value can only be validated against state-growth benchmarks, and the benchmark suite sits outside the measured blast radius (excluded from default fills) while its state-heavy compositions halve in per-block capacity. |
