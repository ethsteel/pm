...
Link: https://eips.ethereum.org/EIPS/eip-7907
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | New excess code gas (2 gas/word for code >24KB) on cold access. Interacts with EIP-2929 cold/warm tracking. Affects tests measuring exact gas. |
| **Blob gas accounting changes** | 0 |   |
| **New EVM gas refund** | 0 |   |
| **Patterns affecting pre-existing tests** | 2 | Gas calculation tests need updates. Initcode boundary tests at old 48KB limit affected. |
| **Transition-tool interface changes** | 0 |   |
| **Cryptography-related testing** | 0 |   |
| **Edge/boundary conditions** | 2 | Multiple boundaries (24KB, 64KB, 128KB thresholds), cold/warm interaction with excess cost, empty code always warm. |
...
| **Added opcodes** | 0 |   |
| **Modified opcodes** | 0 | Gas costs change for 6 opcodes but behavior unchanged. Per the criterion definition, gas changes don't count as behavior modification. |
| **Added precompiles** | 0 |   |
| **Modified precompiles** | 0 |   |
| **Encoding changes (RLP/SSZ)** | 0 | Account state encoding changes but this criterion covers tx/block/interface encoding, not internal state. |
| **New transaction types** | 0 |   |
| **New or modified transaction validity mechanisms** | 1 | Initcode limit increases from 48KB to 128KB. Existing valid initcode remains valid. |
| **New block / header fields** | 0 |   |
| **New fork activation mechanism** | 0 |   |
| **Performance risks** | 2 | 64KB contracts = 2.67x more bytecode to load, increased JUMPDEST analysis time, larger Merkle proofs. Code loading can be benchmarked in isolation but real-world impact depends on block composition. |
| **Security risks** | 1 | EIP-170's 24KB limit was a DoS mitigation - now relaxed. The 2 gas/word pricing must accurately reflect resource costs; if wrong, enables cheaper DoS than expected. May require validation of gas pricing against actual resource consumption. |
| **Cross-EIP interactions** | 1 | Depends on EIP-2929 cold/warm tracking. Modifies EIP-170 (code size) and EIP-3860 (initcode size) limits. |

**Total: 12**

#### Special Considerations

Account `codesize` field requires fork-aware encoding in both specs and testing framework. Testing framework uses `FrontierAccount` for all forks - needs either a new account class or optional `codesize` field with fork-aware handling. State root calculation changes post-fork. Cross-client `codesize == len(code)` invariant is validated implicitly via state root checks.

#### Notes

Excess code cost only charged on cold access (warm unchanged). Empty code is always warm. Boundary tests needed at 24KB±1, 64KB±1, 128KB±1.

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`12`** |
