...
Link: https://eips.ethereum.org/EIPS/eip-8200
...
|---|---:|---|
| **EVM Gas rule changes** | 1 | No new mechanism: the three precompile-specific gas formulas are deleted and calls are charged ordinary EVM execution costs. The pricing shift itself is scored under "Patterns" and "Modified precompiles". |
...
| **Patterns affecting pre-existing tests** | 3 | Measured: an EELS prototype flips 948 fixture executions across 40 functions in 12 directories spanning byzantium through osaka plus the ported static suites — a diverse, multi-era category, with the precompile benchmark suites additionally affected. Every flip has been diagnosed (gas drift in both directions, invalid-input semantics, warm-set loss): none is unexplained. |
| **New invariant on pre-existing tests** | 0 |    |
| **Transition-tool interface changes** | 0 |    |
| **New test-framework primitives** | 1 | Existing primitives suffice with minor extension: a fork precompile-list subtraction and code pre-allocation at the retired addresses. |
| **Cryptography** | 2 | Three well-known cryptographic functions (RIPEMD-160, modular exponentiation, BLAKE2f) are reimplemented in EVM bytecode; each reimplementation must be differentially proven against vast existing vectors. |
| **Edge/boundary conditions** | 3 | The equivalence surface is the full input domain of three functions: lengths, invalid encodings, and gas boundaries. Measured: the prototype bytecode already diverges on one edge — BLAKE2f's final-block flag `0x02` is accepted where EIP-152 requires rejection — and invalid inputs revert with `Error(string)` where the precompiles return empty revert data. |
| **Block syncing changes** | 0 |    |
| **Engine API changes** | 0 |    |
| **Added system contracts** | 2 | Three stateless code deposits appear at the retired addresses; none triggers a system action. |
| **Modified system contracts** | 0 |    |
| **Added opcodes** | 0 |    |
| **Modified opcodes** | 0 |    |
| **Added precompiles** | 0 |    |
| **Modified precompiles** | 3 | The behavior of three pre-existing precompiles changes: ordinary EVM pricing (measured: small-input MODEXP ~6.6×, `rounds`-priced BLAKE2f callers out-of-gas, and an inverse case where the EIP-7883 formula exceeds the transaction gas cap but the EVM version succeeds), different invalid-input returndata, and loss of EIP-2929 pre-warming at the retired addresses — the last unstated by the EIP and measured as 198 flipped executions. |
...
| **New fork activation mechanism** | 3 | Code is written to three addresses at the start of the activation block — a protocol-mandated install with no deploying transaction, implemented in the prototype through the spec's irregular-state-transition hook (the EIP-8141 expiry-verifier pattern: code only, nonce and balance preserved) plus a genesis pre-allocation for test fixtures. |
| **Performance risks** | 2 | The replacements can be benchmarked in isolation, but the existing precompile benchmark suites target exactly these addresses and must be re-derived; interpreter-bound MODEXP on large inputs is the stress case. |
| **Security risks** | 2 | Consensus-critical bytecode replaces natively audited implementations; an implementation error is a consensus split. The measured BLAKE2f flag divergence is this risk realized in the current candidate bytecode — differential fuzzing against the retired implementations is mandatory. |
| **Unspecified behavior requiring cross-client consensus** | 3 | The consensus artifact itself is unspecified: the EIP ships without the deployment bytecode, with TODO gas tables and TODO test cases. The prototype pins the runtime artifacts from eth-act/evmification's own CI; an independent compile reproduced every executable byte but differed in the CBOR metadata trailer — precisely the divergence that becomes a consensus split unless the EIP pins the exact bytes by hash. Every revision of the eventual bytecode re-baselines the whole differential corpus. |
| **Cross-EIP interactions** (uncapped) | 3 | EIP-7666 (the shared deployment mechanism), EIP-7883/7823 (whose MODEXP pricing and bounds suites the change flips), EIP-2929 (warm-set membership), and EIP-152 (BLAKE2f semantics) each need coordinated vectors — four targets, short of an increment. |

**Total: 28**

#### Special Considerations

The differential corpus already exists — the retired precompiles' own test suites — and running it against the candidate bytecode is the test plan: the prototype run surfaced one consensus divergence and both directions of gas drift before any new test was written.

#### Notes

Scores calibrated against a full EELS prototype: mechanism implemented, candidate bytecode compiled from eth-act/evmification `a6da857`, the fork-activation install implemented via the irregular-transition hook, and the full-suite fill measured (948 flips, all diagnosed; equivalence pins on canonical vectors pass; one consensus bug found in the candidate bytecode — BLAKE2f accepts final-block flag `0x02`).

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`28`** |
