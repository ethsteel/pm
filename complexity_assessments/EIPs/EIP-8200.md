...
| **Patterns affecting pre-existing tests** | 3 | Measured: an EELS prototype (eth-act/evmification bytecode) flips 948 fixture executions across 40 functions in 12 directories spanning byzantium through osaka plus the ported static suites — a diverse, multi-era category, with the precompile benchmark suites additionally affected. |
...
| **New fork activation mechanism** | 3 | Code is written to three addresses at the start of the activation block — a state modification at the fork boundary that transition tooling and genesis handling must special-case. |
...
| **Unspecified behavior requiring cross-client consensus** | 3 | The consensus artifact itself is unspecified: the EIP ships without the deployment bytecode, with TODO gas tables and TODO test cases. Every revision of the eventual bytecode re-baselines the whole differential corpus; the prototype had to source candidate bytecode from a third-party repository (eth-act/evmification). |
...
Scores calibrated against a full EELS prototype: mechanism implemented, candidate bytecode compiled from eth-act/evmification `a6da857`, and the full-suite fill measured (948 flips; equivalence pins on canonical vectors pass).
