...
Link: https://eips.ethereum.org/EIPS/eip-8058
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | EIP wants to make duplicate contract deployment cheap. |
| **Blob gas accounting changes** | 0 |   |
| **New EVM gas refund** | 0 |  |
| **Patterns affecting pre-existing tests** | 3 | Gas cost of existing tests might have to be revisited. |
| **Transition-tool interface changes** | 0  |   |
| **Cryptography-related testing** | 0 |   |
| **Edge/boundary conditions** | 3 | If transaction execution were to be fully parallelized this EIP could lead to critical edge cases.  |
...
| **New transaction types** |  0  | The EIP avoids a new tx type by using the existing access list mechanic.  |
| **New or modified transaction validity mechanisms** | 3 | Transactions that might have run out of gas (or exceed the block gas limit) in the past might now be valid due to decreased contract deployment cost.  |
| **New block / header fields** | 0 |   |
| **New fork activation mechanism** | 0  |   |
| **Performance risks** | 0  |   |
| **Security risks** | 2  | If transaction execution were to be fully parallelized this EIP could lead to critical edge cases.  |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`14`** |
