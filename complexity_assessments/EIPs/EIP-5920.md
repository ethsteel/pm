...
Link: https://eips.ethereum.org/EIPS/eip-5920
...
| **Edge/boundary conditions** | 3 | `PAY` with `value` 0 (e.g. in STATIC context), or `address` zero-address, or `address` being `self` are all edge cases that need to be tested. |
...
| **Added opcodes** | 3 | Adds new opcode `PAY` which aims to be a UX improvement. |
| **Modified opcodes** | 0 | |
| **Added precompiles** | 0 | |
...
| **Performance risks** | 0 |  |
| **Security risks** | 2 | There needs to be some edge-case testing performed.  |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`8`** |
