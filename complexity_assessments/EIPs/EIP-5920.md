...
Link: https://github.com/ethereum/EIPs/blob/3b3fb63e8a53a92c771fa0f923d312d87d1a321e/EIPS/eip-5920.md
...
|---|---:|---|
| **EVM Gas rule changes** | 0 | No new mechanism. PAY composes existing ones: warm/cold access, `GAS_NEW_ACCOUNT`, `GAS_CALL_VALUE`. Its dynamic cost is scored under Added opcodes. |
| **State-access ordering within opcode execution** | 2 | New state-accessing operation whose order must be settled. PAY's gas depends on whether `addr` exists, read before the transfer is known to succeed. Unspecified: whether `addr` enters the BAL when PAY returns 0 for insufficient balance, and whether `GAS_NEW_ACCOUNT` is still charged for an account never created. |
| **Blob gas accounting changes** | 0 |  |
| **State gas accounting changes** | 2 | New state-gas charging site. PAY creates an account when `addr` is absent and `val` non-zero, costing `StateGasCosts.NEW_ACCOUNT` (183,600 state gas) plus `ACCOUNT_WRITE` (9,000 execution gas). The EIP's gas table has no state-gas term, so this branch is unpriced. |
| **New EVM gas refund** | 0 |  |
| **Patterns affecting pre-existing tests** | 0 | Purely additive; no existing test changes. |
| **New invariant on pre-existing tests** | 0 |  |
| **Transition-tool interface changes** | 0 |  |
| **New test-framework primitives** | 0 | Existing primitives suffice: balance expectations and BAL balance-change primitives already exist. |
| **Cryptography-related testing** | 0 |  |
| **Edge/boundary conditions** | 2 | Multiple boundary-prone mechanisms, none needing elevated cases: `val` 0, zero-address, `addr` equal to self, the two exceptional halts (high 12 bytes set, static frame), and the gas tree over warm/cold, `addr` exists or not, and `val` zero or not. All within one opcode, so 2 rather than the revision-1 score of 3. |
| **Block syncing changes** | 0 |  |
| **Engine API changes** | 0 |  |
| **Added system contracts** | 0 |  |
| **Modified system contracts** | 0 |  |
| **Added opcodes** | 2 | One new complex opcode: `PAY` (`0xfc`), complex via its dynamic gas cost. Was 3 under revision 1; level 3 requires multiple opcodes. |
| **Modified opcodes** | 0 |  |
| **Added precompiles** | 0 |  |
...
| **Performance risks** | 0 |  |
| **Security risks** | 2 | Force-funding without calling is already possible via `SELFDESTRUCT` and coinbase priority fees, so no invariant is newly broken, but `PAY` makes it much cheaper. |
| **Unspecified behavior requiring cross-client consensus** | 2 | Predates three live Amsterdam mechanisms and requires an update: whether `PAY` emits an EIP-7708 transfer log, how its accesses and balance changes enter the EIP-7928 BAL, and how account creation is priced under EIP-8037. Self-directed PAY (`addr` equal to the current address) is also unstated. |
| **Cross-EIP interactions** (uncapped) | 2 | Five interacting EIPs, each individually simple: **EIP-7523** (hard precondition — PAY "cannot be implemented on networks with empty accounts"), **EIP-7708** (should emit a transfer log), **EIP-7928** (BAL entries), **EIP-8037** (state gas), **EIP-6780** (cited as the existing force-send route). Was 0 under revision 1. |
...
- **The account-creation branch is unpriced** — 183,600 state gas plus 9,000 execution gas that the EIP's gas table predates. Re-derive the gas section before writing vectors.

#### Notes

- Revision 1 to 2 diff. Moved: **Added opcodes 3 to 2**, **Edge/boundary conditions 3 to 2**, **Cross-EIP interactions 0 to 2**. New rows scored: **State-access ordering 2**, **State gas accounting 2**, **Unspecified behavior 2**. New rows at 0: New invariant on pre-existing tests, New test-framework primitives. **Security risks** keeps 2, but its revision-1 rationale ("There needs to be some edge-case testing performed") justified edge cases rather than a security concern and was replaced. The removed **Engine API encoding changes** row was 0.
- The EIP says contracts should not rely on the high-bytes exceptional halt, since Address Space Extension may change it. Assert the halt without encoding it as permanent.
