...
| **Edge/boundary conditions** | 2 | Multiple edge-prone but bounded cases: sweep-to-self (balance now always preserved), zero-balance sweep to a dead beneficiary (no creation, no state gas), SENDALL in initcode, repeated SENDALL of the same account in one tx, CREATE2 onto a now-persistent account (collision path). |
...
| **Performance risks** | 1 | Removes work (end-of-tx account clearing). The opcode benchmarks in isolation; existing SELFDESTRUCT / state-clearing benchmarks change meaning but not baselines elsewhere. |
...
| **Unspecified behavior requiring cross-client consensus** | 2 | Stagnant two-line 2022 text predates EIP-6780/7708/8037/8246; no client implementations or devnet. Localized details need agreement before baselining: EIP-7708 transfer-log emission for the self-sweep, SENDALL's state-gas charging sites under EIP-8037, and confirmation that halt semantics are retained. |
| **Cross-EIP interactions** (uncapped) | 2 | Supersedes EIP-6780's deletion gate and EIP-8246's tombstone (their suites are re-derived), re-derives those tests' EIP-7928 BAL vectors, settles the EIP-7708 self-sweep log, and confirms EIP-8037 charging sites. Five interacting EIPs (6780, 8246, 7928, 7708, 8037), each interaction narrow; no +1 increment (under six). |

**Total: 12**

#### Special Considerations

The score sits on the 🟢/🟡 boundary, and almost none of it is test volume: the automatic 3 for modifying a pre-existing opcode, plus respec/coordination cost, carry the total. Measured fill impact of removing deletion in EELS is small because EIP-6780 already made deletion rare.
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`12`** |
