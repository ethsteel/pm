...
Link: https://eips.ethereum.org/EIPS/eip-8070
...
| **Patterns affecting pre-existing tests** | 1 | Blob tests might have to be revisited to account for cases in which nodes probabilistically act as sampler instead of provider. |
| **Transition-tool interface changes** | 0  |   |
| **Cryptography-related testing** | 0 |   |
| **Edge/boundary conditions** | 0 |  |
| **Block syncing changes** |  0  |   |
| **Engine API changes** | 3  | The EIP proposes Engine API extensions (`engine_blobCustodyUpdatedV1` and `engine_getBlobsV4`).  |
...
| **New or modified transaction validity mechanisms** | 1 | Type 3 transactions should only be valid if the blob data is actually available, the proposed EIP makes it slightly more complicated to verify that this is the case. |
| **New block / header fields** | 0 |   |
| **New fork activation mechanism** | 0  |   |
| **Performance risks** | 0  |   |
| **Security risks** | 2  | The probabilistic sampling approach (and added sampling noise) have to be thoroughly verified and limit-tested in devnets and testnets. There is a non-zero chance with `p=0.15` that a blob is not available to anyone.  |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`7`** |
