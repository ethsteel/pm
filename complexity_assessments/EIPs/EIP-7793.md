...
Link: https://eips.ethereum.org/EIPS/eip-7793
...
| **Edge/boundary conditions** | 3 | Transaction validity now also depends on slot/index.  |
...
| **Added opcodes** | 3 | Adds new opcode `TXINDEX`.  |
| **Modified opcodes** | 0 |  |
| **Added precompiles** | 0 |   |
| **Modified precompiles** | 0 |   |
| **Encoding changes (RLP/SSZ)** | 0 |   |
| **New transaction types** | 3 | Introduces new `2728` tx type called 'conditional transactions'.  |
| **New or modified transaction validity mechanisms** | 3 + 3 | Transaction validity now also depends on slot/index. Added double weight for this reason.  |
| **New block / header fields** | 0 |   |
| **New fork activation mechanism** | 0 |   |
| **Performance risks** | 0 |  |
| **Security risks** | 3 + 3 | Transaction validity now also depends on slot/index, this has not done before and can potentially lead to consensus issues due to the overall complexity this EIP brings, therefore double weight added.  |
