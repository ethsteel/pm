...
Link: https://eips.ethereum.org/EIPS/eip-7708
...
| **Patterns affecting pre-existing tests** | 1 | All tests will now issue Eth-transfer logs, which might be a non-issue but does touch basically every single test. |
| **Transition-tool interface changes** | 1 | Rather than a change to the T8N interface, we have to enhance execution-specs testing framework to verify the logs returned from T8N, because at the moment we just pass along the log list that was received from T8N to the test to be compared against the clients. |
| **Cryptography-related testing** | 0 |   |
| **Edge/boundary conditions** | 2 | All 0->1 boundaries have to be tested in all possible scenarios where Eth is transfered.  |
| **Block syncing changes** | 0 |   |
| **Engine API changes** | 0 |   |
| **Engine API encoding changes** | 0 |   |
| **Added system contracts** | 0 |   |
| **Modified system contracts** | 2 | Some system contracts do receive/transfer Eth, and while existing tests might be sufficient, we need to double check that this change does not break current behavior (particularly the beacon deposit contract). |
| **Added opcodes** | 0 |   |
| **Modified opcodes** | 1 | *CALL opcodes and SELFDESTRUCT/SENDALL behavior is slightly modified. |
| **Added precompiles** | 0 |   |
...
| **Performance risks** | 2 | Test worst-case scenario of a transaction/block transfering Eth in multiple ways. |
| **Security risks** | 0 |   |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | 9 |
