...
Status at assessment time: **Draft** (created 2024-05-18, `FORK_TIMESTAMP` TBD, `requires: 2935`).
Scored assuming activation in a fork where EIP-7928 (BAL) is already live.
...
|---|---:|---|
| **EVM Gas rule changes** | 1 | `BLOCKHASH` opcode gas costs now follow state access rules, accounting for cold/warm scenarios. This affects existing `BLOCKHASH` test cases but not other pre-existing scenarios. Given 1 point consider the impact should be limited. |
| **State-access ordering within opcode execution** | 2 | Only `BLOCKHASH` being impacted and now accounting for state access cost.   |
...
| **New EVM gas refund** | 0 | No new gas-refund mechanisms are introduced. |
| **Patterns affecting pre-existing tests** | 2 | Most of the `BLOCKHASH` related tests need refactoring, but impact is limited (27 places under tests/ folder, excluding `tests/benchmark`) |
| **New invariant on pre-existing tests** | 1 | Pre-existing tests that call `BLOCKHASH` for an in-window ancestor must now assert a storage-read entry for `HISTORY_STORAGE_ADDRESS` in the BAL and the post-call warmth of that slot—even though these tests aren't about access lists. This narrowly affects only tests that already touch `BLOCKHASH` |
| **Transition-tool interface changes** | 0 | No interface changes  |
| **New test-framework primitives** | 1 | Add storage-access tracking to `BLOCKHASH`. Follow `Op.SLOAD(key_warm=...)` and integrate into fork gas map.  |
| **Cryptography-related testing** | 0 | No cryptography mechanism added  |
| **Edge/boundary conditions** | 2 | Multiple interacting edge cases: (1) window boundary (`arg == block.number`, `> block.number`, `== 2**256-1`, `-1`, `-256 vs -257`), (2) young chain (`block.number < 256`, cold charge on unwritten slots), (3) modulus mismatch (`8191` vs `256`, `EIP-2935` slot warming), (4) cold/warm state (`EIP-2930` access lists, `SLOAD` collisions), (5) revert rollback and OOG at cold-charge boundary, (6) fork-transition window.  |
| **Block syncing changes** | 0 | No block syncing changes   |
| **Engine API changes** | 0 | No engine API changes  |
| **Added system contracts** | 0 | No system contracts added  |
| **Modified system contracts** | 1 | The history contract's code and state are not directly modified, but there's an indirect effect: `BLOCKHASH` warms the contract's storage slot, making subsequent `SLOAD` calls within `get()` warm. Conversely, `SLOAD` on that slot warms it for `BLOCKHASH`. |
| **Added opcodes** | 0 | No opcode added |
| **Modified opcodes** | 1 | BLOCKHASH now has state-access side effects: slot warming and state-access records, observable independently of gas. At activation, in-window lookups return 0 if < 256 blocks since EIP-2935 fork (vs pre-fork real hash). |
| **Added precompiles** | 0 | No added precompile |
| **Modified precompiles** | 0 | No modified precompile  |
| **Encoding changes (RLP/SSZ)** | 0 | No encoding rules changes  |
| **New transaction types** | 0 | No transaction types added  |
| **New or modified transaction validity mechanisms** | 0 | No changes to transaction validity mechanisms |
| **New block / header fields** | 0 | No new block or header fields are introduced.  |
| **New fork activation mechanism** | 0 | No state modifications, internal variables or similar are modified at the fork activation block.  |
| **Performance risks** | 1 | The new work reduces to an `SLOAD`, which is already benchmarked and already priced; the EIP strictly raises the cost of the operation, so worst-case throughput moves in the safe direction. |
| **Security risks** | 1 | 106x cost increase might break `BLOCKHASH` callers. Interacts with EIP-2929/BAL. "MAY" clause risks consensus divergence if warming/recording skipped. Needs targeted review and fuzz testing on state-access accounting. |
| **Unspecified behavior requiring cross-client consensus** | 2 | (1) does the lookup add HISTORY_STORAGE_ADDRESS to accessed_addresses? (2) whether the account appears in the BAL account list when only a slot is read through the opcode, and whether the access is recorded when the opcode OOGs on the cold charge. |
| **Cross-EIP interactions** (uncapped) | 2 | 4 interacting EIPs: EIP-2935 (hard dependency, ring-buffer state), EIP-2929 (cold/warm foundation), EIP-2930 (access list pre-warming), EIP-7928 (BAL recording). Test vectors need redesign.  |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`17`** |
