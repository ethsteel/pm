...
Link: https://eips.ethereum.org/EIPS/eip-7745
...
| **Patterns affecting pre-existing tests** | 2 | The framework needs to be updated to handle the proposed new lookup index data structure. |
| **Transition-tool interface changes** | 2 | The t8n might also have to be adjusted.  |
| **Cryptography-related testing** | 2 | It is unclear how computationally-expensive it is to mine collisions in the proposed lookup index data structure and which performance impact such collisions would have in practice. |
| **Edge/boundary conditions** | 0  |   |
| **Block syncing changes** | 3 | The `logs_bloom` header field must be replaced with `log_index_root`.  |
...
| **Encoding changes (RLP/SSZ)** | 3 | The `logs_bloom` header field must be replaced with `log_index_root`.  |
| **New transaction types** | 0 |  |
| **New or modified transaction validity mechanisms** | 0 |  |
| **New block / header fields** | 3 | The `logs_bloom` header field must be replaced with `log_index_root` and the proposed logic must be implemented to calculate that field.  |
| **New fork activation mechanism** | 0 |   |
| **Performance risks** | 2 | `LOG` performance might be negatively impacted which is why the author predicts a gas increase of this opcode class. It is known that it is possible to mine collisions in the proposed lookup index data structure and it is unclear, until benchmarked, which performance impact such collisions would have in practice. |
| **Security risks** | 2 | Requires extensive testing in testnets and devnets to avoid consensus issues. |
| **Cross-EIP interactions** | 1 | `EIP-7668` tries to remove the bloom filter, and this EIP tries to replace it with something else. |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | `20` |
