...
EIP-7971 reprices `TLOAD` from 100 to 5 gas and `TSTORE` from 100 to
12 gas, with a 24-gas surcharge on the first write to each transient
`(address, key)` tuple in a transaction. It also limits a transaction to
131,072 unique transient-slot writes. Allocation state is shared across
CALL and CREATE/CREATE2 frames, rolled back with a reverted or halted
frame, and reset between transactions.

The finalized EELS reference prototype is
[spencer-tb/execution-specs#58](https://github.com/spencer-tb/execution-specs/pull/58).
It implements the change in Amsterdam and activates it through the testing-side
Bogota pseudo-fork.
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | `TLOAD` and `TSTORE` are repriced and `TSTORE` gains a first-write allocation charge. The new dynamic rule affected the existing EIP-1153 gas map and tests. |
| **State-access ordering within opcode execution** | 0 | Transient storage is excluded from the block access list, and the EIP does not move any persistent state access or its gas charge. |
| **Blob gas accounting changes** | 0 | No blob gas accounting changes. |
| **State gas accounting changes** | 0 | The change affects EVM execution gas, not Amsterdam state gas or its reservoir/spill mechanism. |
| **New EVM gas refund** | 0 | No gas-refund mechanism is introduced. |
| **Patterns affecting pre-existing tests** | 1 | The Bogota full fill initially flipped 18 executions across four existing test functions. Fork-correct fixes covered EIP-8038, EIP-1153, generic opcode metering, EIP-7702 depth, and two transient-storage benchmark functions; no tests were parked. |
| **New invariant on pre-existing tests** | 0 | Unrelated tests gain no new assertion; the affected tests were reworked for the new gas rule. |
| **Transition-tool interface changes** | 0 | The unique-slot set is internal execution state and requires no t8n field or interface change. |
| **New test-framework primitives** | 1 | Existing primitives suffice after minor extensions: a fork slot-limit API, an allocation gas constant, and `slot_allocated` opcode metadata. |
| **Cryptography-related testing** | 0 | No cryptographic mechanism is introduced or modified. |
| **Edge/boundary conditions** | 3 | Tests must combine exact/one-over slot limits, exact/one-short gas, fresh/repeated/zero writes, address scoping, frame success/revert/halt, CALL and CREATE/CREATE2 contexts, reads, and transaction resets. Several cases execute 131,072-slot loops. |
| **Block syncing changes** | 0 | No block RLP or syncing rule changes. |
| **Engine API changes** | 0 | No Engine API change. |
| **Added system contracts** | 0 | No system contract is introduced. |
| **Modified system contracts** | 0 | No existing system contract code, state, or demonstrated behavior is modified. |
| **Added opcodes** | 0 | No opcode is added. |
| **Modified opcodes** | 3 | Beyond gas repricing, `TSTORE` gains a new exceptional-halt condition when the transaction-global allocation limit is exceeded. |
...
| **New or modified transaction validity mechanisms** | 0 | Exceeding the limit is an execution halt, not transaction invalidity or an intrinsic-gas change. |
| **New block / header fields** | 0 | No block or header field is introduced. |
| **New fork activation mechanism** | 0 | The allocation set is initialized per transaction; no state or internal variable is modified at the activation block. |
| **Performance risks** | 2 | Isolated TLOAD/TSTORE benchmarks exist, but the transaction-global limit and frame rollback need end-to-end validation. The unique-key benchmark had to be split at 131,072 writes per successful transaction. |
| **Security risks** | 2 | The limit is the EIP's memory-exhaustion defense and interacts with shared transaction state plus frame journaling, requiring targeted review and fuzzing of rollback and bypass paths. |
| **Unspecified behavior requiring cross-client consensus** | 2 | Localized consensus decisions remain: whether exceeding the limit halts the frame or whole transaction, whether the allocation surcharge is normative, and whether zero-value writes allocate. |
| **Cross-EIP interactions** (uncapped) | 2 | Coordinated coverage is required for EIP-1153 transient semantics, EIP-7702 call-depth/context behavior, EIP-7825's transaction cap, and EIP-8038's related storage repricing. The interactions are limited in scope; four EIPs do not trigger the uncapped increment. |

**Total: 19**

#### Special Considerations

The matrix grows across allocation state (fresh/repeated), value (zero/non-zero),
address and execution context (CALL, CALLCODE/DELEGATECALL, CREATE/CREATE2),
frame outcome (success/revert/exceptional halt), and the exact slot and gas
boundaries. The additive score does not fully represent those combinations.

#### Notes

The final prototype's full Bogota fill before review had 65,492 passing and 294
skipped executions with no failures. Its initial run flipped 18 executions in
four existing functions; all were fixed fork-correctly with no EIP-7971 parking.
Review added transaction-reset coverage, CALL plus CREATE/CREATE2 global-limit
coverage, and corrected a unique-key benchmark that previously passed by
exceptionally halting above the slot cap. The complete transition test remains
skipped until a real post-Amsterdam spec fork exists.

The EIP revision pinned by the tests is
[`de83a8d1`](https://github.com/ethereum/EIPs/blob/de83a8d1bcdf89a5780076290b1d9a0799f2faec/EIPS/eip-7971.md).
No newer amendment changes that file.

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`19`** |
