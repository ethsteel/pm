...
Link: https://eips.ethereum.org/EIPS/eip-7971

## Execution Specs

### Specs

EIP-7971 modifies transient storage (EIP-1153) by reducing gas costs and introducing a hard limit on transient storage allocation per transaction:

- **Gas Cost Changes:**
  - `TLOAD` (0x5c): Reduced from 100 to 5 gas.
  - `TSTORE` (0x5d): Reduced from 100 to 12 gas base cost.
  - `GAS_TSTORE_ALLOCATE`: 24 gas surcharge for writing to a fresh slot (first write to unique slot).

- **Transaction-Global Limit Mechanism:**
  - New counter `transient_slots_used` initialized to 0 at transaction start.
  - Counter increments when TSTORE writes to a new unique slot (based on `(contract_address, storage_key)` tuple).
  - Counter persists across all message calls within a transaction (including nested calls).
  - `MAX_TRANSIENT_SLOTS`: 131,072 unique slots per transaction.
  - Transaction halts with exceptional halt if limit is exceeded.
  - Counter resets at transaction completion.
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | New gas accounting for TLOAD/TSTORE with dynamic allocation cost (GAS_TSTORE_ALLOCATE=24 for fresh slots). Affects existing EIP-1153 gas mechanisms and will require updates to existing transient storage tests. |
| **Blob gas accounting changes** | 0 | No blob gas changes. |
| **New EVM gas refund** | 0 | No gas refund mechanism introduced. |
| **Patterns affecting pre-existing tests** | 2 | Existing EIP-1153 test suite substantially affected: gas constants in `spec.py` need updating, gas-dependent tests in `test_tstorage_execution_contexts.py` will break, and failure mode tests need extension for new MAX_TRANSIENT_SLOTS halt condition. |
| **Transition-tool interface changes** | 0 | No t8n interface changes required; transient_slots_used is internal EVM state. |
| **Cryptography-related testing** | 0 | No cryptographic mechanisms introduced. |
| **Edge/boundary conditions** | 2 | Multiple boundaries: MAX_TRANSIENT_SLOTS limit (131,072), slot uniqueness tracking across contracts and nested calls, fresh vs existing slot allocation costs, counter persistence across message calls. |
| **Block syncing changes** | 0 | No RLP validation changes. |
| **Engine API changes** | 0 | No Engine API modifications. |
| **Engine API encoding changes** | 0 | No Engine API encoding changes. |
| **Added system contracts** | 0 | No system contracts introduced. |
| **Modified system contracts** | 0 | No system contract modifications. |
| **Added opcodes** | 0 | No new opcodes; TLOAD/TSTORE already exist from EIP-1153. |
| **Modified opcodes** | 3 | TSTORE has a new failure mode: exceptional halt when transaction-global counter exceeds MAX_TRANSIENT_SLOTS. This is a behavioral change beyond gas costs - existing tests cover REVERT/INVALID/OOG/stack failures, but not this new limit-based halt. |
| **Added precompiles** | 0 | No precompiles introduced. |
| **Modified precompiles** | 0 | No precompile modifications. |
| **Encoding changes (RLP/SSZ)** | 0 | No encoding changes. |
| **New transaction types** | 0 | No new transaction types. |
| **New or modified transaction validity mechanisms** | 0 | No transaction validity changes; the slot limit causes exceptional halt during execution, not transaction rejection. |
| **New block / header fields** | 0 | No new block or header fields. |
| **New fork activation mechanism** | 0 | Counter initialized at transaction start (not fork activation); per template note, initialization is not a modification. |
| **Performance risks** | 2 | Significantly reduced gas costs (95% reduction for TLOAD) enable more transient storage operations per block. Needs benchmarking to validate DoS resistance with the 131,072 slot limit. |
| **Security risks** | 2 | The slot limit is a DoS prevention mechanism that interacts with EVM execution. Incorrect implementation could allow memory exhaustion attacks. Requires targeted security review. |
| **Cross-EIP interactions** | 2 | Directly modifies EIP-1153 gas costs and adds constraints. Requires coordinated testing with existing transient storage behavior; existing EIP-1153 test vectors need updating. |

**Total: 16**

#### Special Considerations

The EIP notes that gas cost values are not yet finalized pending benchmarking studies. Test implementations should be designed to easily update gas constants once final values are determined. The relationship to EIP-8038 (warm storage costs) should be monitored for potential alignment.

#### Notes

**Existing test impact (execution-specs `tests/cancun/eip1153_tstore/`):**
- `spec.py`: Gas constants `TLOAD_GAS_COST=100` and `TSTORE_GAS_COST=100` need updating to 5 and 12/36 respectively.
- `test_tstorage_execution_contexts.py`: Tests use `Spec.TSTORE_GAS_COST` to create precise out-of-gas conditions (line 189). These will break because 100 gas is now excessive for TSTORE.
- Existing failure mode tests (REVERT, INVALID, stack overflow/underflow, OOG) need extension to cover the new MAX_TRANSIENT_SLOTS exceptional halt.

**Counter tracking requirements:**
- Verify unique `(contract_address, storage_key)` tuple tracking across nested calls (CALL, DELEGATECALL, STATICCALL).
- DELEGATECALL: slot keyed by caller's address, so DELEGATECALL TSTORE to a slot caller already wrote should NOT increment counter.
- Edge cases: exactly MAX_TRANSIENT_SLOTS writes, exceeding by 1, multiple contracts writing to same key (count separately), same contract same key multiple times (count once).

**Contract creation contexts:** CREATE/CREATE2 MUST inherit the parent's `transient_slots_used` counter. Otherwise, a transaction could bypass the limit by creating contracts that each allocate their own slots. Test that creator + created contract share the same counter.

**STATICCALL:** Confirm TSTORE remains invalid in static context (existing EIP-1153 behavior).

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | **`16`** |
...
N/A - This is an execution layer only EIP.
