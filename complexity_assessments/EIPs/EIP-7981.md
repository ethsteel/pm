...
Link: https://eips.ethereum.org/EIPS/eip-7981
...
|---|---:|---|
| **EVM Gas rule changes** | 2 | It's a small change but we also have to update the intrinsic gas cost calculator and probably its interface used in many access list tests to include checking the bytes in them. |
| **Blob gas accounting changes** |   |   |
| **New EVM gas refund** |   |   |
| **Patterns affecting pre-existing tests** | 2 | We have to update tests that rely on the exact intrinsic gas cost of the transactions and use access lists. Mid complexity because number of tests is not that many.  |
| **Transition-tool interface changes** |   |   |
| **Cryptography-related testing** |   |   |
| **Edge/boundary conditions** | 1 | Checks for the type of byte require boundary condition checks but minimal complexity. |
| **Block syncing changes** |   |   |
| **Engine API changes** |   |   |
...
| **Encoding changes (RLP/SSZ)** |   |   |
| **New transaction types** |   |   |
| **New or modified transaction validity mechanisms** | 1 | Modifies the intrinsic gas cost calculation. |
| **New block / header fields** |   |   |
| **New fork activation mechanism** |   |   |
| **Performance risks** |   |   |
| **Security risks** |   |   |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all criterion scores (0–72) | 6 |
