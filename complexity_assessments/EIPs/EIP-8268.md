...
| **Patterns affecting pre-existing tests** | 1 | Measured on the EELS implementation (execution-specs#3388): every BAL vector re-derives mechanically at fill time, and zero existing test files needed modification — the declarative expectations and the invalid-BAL suite pass untouched. |
...
| **Block syncing changes** | 1 | A single simple mechanism: the canonical conditional encoding. Client-side validation is the pre-existing `block_access_list_hash` check; the implementation added an encoder, not a validator. |
...
| **Unspecified behavior requiring cross-client consensus** | 0 | The text determined every case the implementation and tests could construct; two independent implementations (execution-specs#3009 and #3388) converged on identical semantics without coordination. |
| **Cross-EIP interactions** (uncapped) | 1 | Modifies EIP-7928's structure but was implemented and tested entirely independently; the EIP-7702 rule needs no code, and the EIP-8146/8279 interactions are coordination notes, not test matrices. No +1 increment. |
...
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`14`** |
