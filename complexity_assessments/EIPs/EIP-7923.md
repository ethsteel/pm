...
Link: https://github.com/ethereum/EIPs/blob/5dec0faf808d8638b3db63afd55c57f60eef82f3/EIPS/eip-7923.md
...
|---|---:|---|
| **EVM Gas rule changes** | 3 | Deletes the linear and quadratic memory terms and replaces them with a 100-gas charge per newly touched 4 KiB page. Repricing reaches all 18 memory-expanding opcodes, and page 0 being free makes `MSTORE(0, x)` cost 3 instead of 6. |
| **State-access ordering within opcode execution** | 0 |  |
| **Blob gas accounting changes** |   |   |
| **State gas accounting changes** |   |   |
| **New EVM gas refund** |   |   |
| **Patterns affecting pre-existing tests** | 3 | 1982 test files pair a memory-expanding opcode with an exact-gas expectation: 1831 in `tests/ported_static/`, 151 hand-written across 14 forks plus `tests/benchmark/`. Also, a single MSTORE(x, 1) can no longer cause an OOG, which is one of the go-to ways that tests use to produce one. |
| **New invariant on pre-existing tests** | 0 |  |
| **Transition-tool interface changes** |   |   |
| **New test-framework primitives** | 3 | `MemoryExpansionGasCalculator` is `(new_bytes, previous_bytes) -> int`, a pure function of sizes. Page cost depends on which offsets are touched and on what the frame touched before, so the protocol, the `new_memory_size`/`old_memory_size` opcode metadata and its six per-EIP consumers all need a page-aware replacement that stays fork-scoped. |
| **Cryptography-related testing** |   |   |
| **Edge/boundary conditions** | 3 | Memory allocation change from continuous to sparse: an instruction pays only for the pages its own range covers, so touching page 1000 leaves pages 1-999 unallocated. On top of that, page-crossing reads and writes on 18 opcodes, free page 0 against paid page 1, first touch against re-touch, the 2**32 address halt, the 16384-page transaction halt, and `MCOPY` with source and destination straddling different pages. |
| **Block syncing changes** |   |   |
| **Engine API changes** |   |   |
| **Added system contracts** |   |   |
| **Modified system contracts** |   |   |
| **Added opcodes** |   |   |
| **Modified opcodes** | 0 | Row excludes gas changes, accounted for in the first row. |
| **Added precompiles** |   |   |
| **Modified precompiles** |   |   |
| **Encoding changes (RLP/SSZ)** |   |   |
| **New transaction types** |   |   |
...
| **New block / header fields** |   |   |
| **New fork activation mechanism** |   |   |
| **Performance risks** | 2 | Memory benchmark tests need to be written from the ground up to cover the new memory allocation mechanism. |
| **Security risks** | 1 | Small risk: contracts that previously ran out of gas by memory allocation, potentially now will not, therefore it's worth exploring if this could be an issue with currently existing contracts. |
| **Unspecified behavior requiring cross-client consensus** |  |  |
| **Cross-EIP interactions** (uncapped) | 0 |  |

**Total: 15**

#### Special Considerations

The page-aware cost model is the long pole, not the test writing. It has to be fork-scoped so that pre-activation forks keep the quadratic calculator, because fork-parametrized tests that pin memory gas run through both models in the same session.

`tests/ported_static/` holds 1831 of the 1982 affected files. Those are machine-ported and excluded from churn attribution in the calibration model, but they still have to pass, and their expected gas is baked into committed fixtures rather than computed at fill time.

Sequencing against EIP-8141 and EIP-7906 matters. Both introduce frame kinds inside a single transaction, and the transaction-global page counter is shared across them. Settling the counter semantics before either lands avoids re-deriving the limit tests twice.

#### Notes

- The reference implementation cannot be read as authoritative on the unwind: `assert transaction_context.num_pages == num_pages_anchor + allocated pages` is a syntax error, and `computation` is unbound in the `finally` when `apply_computation` raises. That `finally` is the only place the unwind is stated at all.
- The Security Considerations derive 54 MiB of reachable memory from a 30M gas limit. EIP-7825 caps transaction gas at 2**24 in Amsterdam, so the reachable figure is lower than the EIP assumes. Settle it with the existing benchmark suite rather than re-deriving the arithmetic.
- `MSIZE` stays a byte high-water mark while cost tracks a sparse page set, so the two decouple hard. `MSTORE(0xFFFFFFE0, 0)` pays for exactly one page and reports an `MSIZE` of 4 GiB, at 103 gas against roughly 3.5e13 today. Inflating `MSIZE` becomes free, which the EIP addresses only for new contracts ("should not rely on `MSIZE` for memory allocation") and not for deployed ones that use it as a free-memory pointer.
- Spec and reference implementation disagree at the very top of the address space. The text halts when an address is "larger than `2**32 - 1`", but `write()` raises on `start_position + size >= 2**32`, which rejects the last legal word at `0xFFFFFFE0`. One vector settles it.
- The 2**32 address halt is unreachable today — expanding to 4 GiB costs roughly 3.5e13 gas under the quadratic term — so no existing vector observes the change.

#### Final Assessment

| Category | Description | Value |
|-----------|--------------|:----:|
| **Total Score** | Sum of all anchor scores (0–84 nominal; **Cross-EIP interactions** is uncapped, so there is no hard maximum) | **`15`** |
