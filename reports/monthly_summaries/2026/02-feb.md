...
- Account access and storage operation benchmarks (SLOAD/SSTORE) identified as the biggest bottleneck. Rahul (Prototyping) has PRs for storage operation benchmarks and will create another for account access. Tests must differentiate cached vs. non-cached scenarios and use deterministic bloated contract slots.
